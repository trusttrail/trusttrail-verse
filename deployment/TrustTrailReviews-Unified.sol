...
/**
 * @title TrustTrailReviews - Unified Contract
 * @dev Combined ERC20 Token + Review Platform + Staking System
 * This contract handles TRST token management AND review platform functionality
 */
contract TrustTrailReviews is ERC20, ERC20Burnable, AccessControl, ReentrancyGuard, Pausable, Ownable {
    ...
    function addComment(uint256 _reviewId, string memory _content) external whenNotPaused returns (uint256) {
        require(_reviewId < nextReviewId, "Review does not exist");
        require(reviews[_reviewId].status == ReviewStatus.Approved, "Review not approved");
